...
class PostController {
    ...
    getPostById = async (req, res) => {
        try {
            ...
            //const fountPost = await post.findById(id) -> COMO ERA ANTES;
            const foundPost = await this.postService.getPostById(id);
            if (!foundPost) {
                return res.status(404).send("Post não encontrado!");
            }
            //res.status(200).json(foundPost) -> COMO ERA ANTES;
            res.status(200).json(new PostDto(foundPost));
        } catch (error) {
    ...
    updatePost = async (req, res) => {
        try {
            const id = req.params.id;
            const updatePost = await this.postService.updatePost(id, req.body, {
                new: true
            });
            res.status(200).json({
    ...
    findPostByKeyword = async (req, res) => {
        try {
            const { keyword } = req.params;
            const result = await this.postService.searchPostBykeyword(keyword);
            if (result.lenght === 0) {
                return res.status(404).json({
                    message: "Nenhum post encontrado com a palavra-chave informada!",
                })
            }
            res.status(200).json(result.map((post) => new PostDto(post)));
        } catch (error) {
            res.status(500).send(error.message);
        }
    }
